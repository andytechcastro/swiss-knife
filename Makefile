test:
	mkdir cover && go test ./kubernetes/builders -v -cover -coverprofile=cover/cov_builders.out && go test ./kubernetes/actions -v -cover -coverprofile=cover/cov_actions.out && rm -r cover
